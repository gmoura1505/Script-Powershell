...
DescriÃ§Ã£o: 	Este script exporta o relatÃ³rio de status do Microsoft 365 MFA para CSV
Version:    1.0
...
<# Exemplos de filtros:
 ...
 Como administrador do Office 365, muitas vezes você pergunta 'Como verificar se o MFA está habilitado no escritório 365'? A solução está aqui. Usando o parâmetro (EnabledOnly), você pode exportar usuários habilitados para MFA para arquivo CSV.

 ./GetMFAStatus.ps1 -EnabledOnly

 

Alguns usuários podem habilitar mfa mas não forçados (processo de registro não concluído) para MFA. Você pode obter uma lista de usuários aplicados do MFA usando -EnforcedOnly .

 ./GetMFAStatus.ps1 -EnforcedOnly

  

O MFA fornece um nível adicional de segurança às contas. Para exibir usuários com deficiência do MFA, você pode executar este script com -DisabledOnly .

 ./GetMFAStatus.ps1 -DisabledOnly

  

Como as contas administrativas têm mais privilégios, requer atenção especial. De acordo com uma pesquisa recente, 78% dos administradores da Microsoft 365 não ativam o MFA para suas contas. Para encontrar administradores sem autenticação multifatorial, execute o script usando –AdminOnly .

 ./GetMFAStatus.ps1 -AdminOnly -DisabledOnly

 

 Em vez de gerar relatório de status MFA para todos os usuários, você pode obter status MFA apenas para usuários licenciados. Você pode usar –LicensedUserOnly param para obter o status MFA dos usuários licenciados  

 

Para exibir o status de ativação do MFA para usuários licenciados,

./GetMFAStatus.ps1 -LicensedUserOnly

  

Para visualizar todos os usuários licenciados que não configuraram MFA,

./GetMFAStatus.ps1 -LicensedUserOnly -DisabledOnly

  

A maioria das organizações mantém as contas dos ex-funcionários em um estado de deficiência. Então, temos -SignInAllowed param, para filtrar o resultado com base no status SignIn,

Para visualizar o login permitido usuários sem MFA

./GetMFAStatus.ps1 -SignInAllowed $True -DisabledOnly

Para listar usuários de login negados com MFA,
...
Nota:
 ...
 Você pode usar vários filtros juntos para obter um relatório de status MFA mais granular. Por exemplo:

 Você pode obter uma lista de usuários habilitados para status MFA cujo status sign-in é negado.

 ./GetMFAStatus.ps1 -EnabledOnly –SignInAllowed $False

    Você pode obter uma lista de usuários de administração desativados do MFA cujo status sign-in é permitido.
